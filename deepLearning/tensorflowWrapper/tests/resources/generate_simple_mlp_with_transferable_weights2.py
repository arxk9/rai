...
with tf.Session() as session:
    ...
    with tf.name_scope('hiddenLayer1'):
        ...
        b1 = tf.Variable(tf.constant(2.0, shape=[hidden_sizes[0]]), name='b')
    ...
    with tf.name_scope('hiddenLayer2'):
        ...
        b2 = tf.Variable(tf.constant(shape=[hidden_sizes[1]], value=2.0), name='b')
    ...
    with tf.name_scope('outputLayer'):
        ...
        bo = tf.Variable(tf.constant(shape=[output_dim], value=2.0), name='b')
    ...
    tf.train.write_graph(session.graph_def, '', 'simple_mlp_with_transferable_weights2.pb', as_text=False)