...
with tf.Session() as session:
    ...
    with tf.name_scope('hiddenLayer1'):
        W1 = tf.Variable(tf.random_uniform(dtype=dtype, shape=[input_dim, hidden_sizes[0]], minval=-1/np.sqrt(input_dim), maxval=1/np.sqrt(input_dim)), trainable=True, name='W')
        b1 = tf.Variable(tf.constant(value=0.1, dtype=dtype, shape=[hidden_sizes[0]]), name='b')
        top = tf.nn.relu(tf.matmul(top, W1) + b1)

    with tf.name_scope('hiddenLayer2'):
        W2 = tf.Variable(tf.random_uniform(dtype=dtype, shape=[hidden_sizes[0], hidden_sizes[1]], minval=-1/np.sqrt(hidden_sizes[0]), maxval=1/np.sqrt(hidden_sizes[0])), name='W')
        b2 = tf.Variable(tf.constant(value=0.1, dtype=dtype, shape=[hidden_sizes[1]]), name='b')
        top = tf.nn.relu(tf.matmul(top, W2) + b2)

    with tf.name_scope('outputLayer'):
        Wo = tf.Variable(tf.random_uniform(dtype=dtype, shape=[hidden_sizes[1], output_dim], minval=-3e-3, maxval=3e-3), name='W')
        bo = tf.Variable(tf.constant(value=0.1, dtype=dtype, shape=[output_dim]), name='b')
    ...
    tf.train.write_graph(session.graph_def, '', 'simple_mlp_float64.pb', as_text=False)

    session.run(initialize_all_variables_op)